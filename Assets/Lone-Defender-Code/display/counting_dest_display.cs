...
public class counting_dest_display : Location_display
{
    ...
    [SerializeField] protected TextMeshProUGUI destination_txt; // Display which clearing these enemies are headed to
    ...
    public override void add_game_piece(Game_piece go)
    {
        ...
        destination_txt.text = new_e.fact.loc.get_id().ToString();

        List<Enemy> loc_e_pawns = loc.get_enemies();
    ...
    public override void remove_game_piece(Game_piece go)
    {
        ...
        {
            factory_dest = -1;
            destination_txt.text = (0).ToString();
        }
